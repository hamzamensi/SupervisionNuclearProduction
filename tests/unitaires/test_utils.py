...
from core.utils import get_sum_value_by_hour, get_nuclear_data, request_data, get_data_from_api, get_token
...
class Test(TestCase):
    ...
    def test_request_data(self):
        ...
        end_date = '2022-01-20T00:00:00+01:00'
        with self.assertRaises(Exception):
            result = request_data(start_date, end_date)
    ...
    def test_get_nuclear_data(self):
        ...
        expected_df = pd.DataFrame(columns=['start_date', 'Sum_per_hour', 'mean_by_hour'],
                                   data=[['2022-12-01T00:00:00+01:00', 408.5, 408.5]])
    ...
    def test_get_data_from_api(self):
        start_date = '2022-01-10T00:00:00+01:00'
        end_date = '2022-01-12T00:00:00+01:00'
        result = get_data_from_api(start_date, end_date)
        min_start_date_result = min([res['start_date'] for res in result])
        self.assertEquals(start_date, min_start_date_result)
        max_end_date_result = max([res['end_date'] for res in result])
        self.assertEquals(end_date, max_end_date_result)
        ...
        # plus de 7 jours
        start_date = '2022-01-10T00:00:00+01:00'
        end_date = '2022-01-22T00:00:00+01:00'
        result = get_data_from_api(start_date, end_date)
        min_start_date_result = min([res['start_date'] for res in result])
        self.assertEquals(start_date, min_start_date_result)
        max_end_date_result = max([res['end_date'] for res in result])
    ...
    def test_get_token(self):
        type_token, token = get_token()
        self.assertIsNotNone(type_token)
        self.assertIsNotNone(token)
        os.environ['ClientId'] = 'OK'
        os.environ['SecretId'] = 'XXXX'
        with self.assertRaises(Exception):
